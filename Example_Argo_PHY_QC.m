...
% download_database("ARGO","PHY",true);

parquetPath = fullfile("./data/1002_PHY_ARGO-QC-DEV-MATLAB/");
...
tic;
dataPHY_QC_SIGMA1 = readall(pds,UseParallel=true);
elapsed = toc;
...
% tic;
% dataPHY_QC_SIGMA1 = readall(pds,UseParallel=false);
% elapsed = toc;
...
[G, LAT, LON] = findgroups(dataPHY_QC_SIGMA1.LATITUDE,dataPHY_QC_SIGMA1.LONGITUDE);
if height(dataPHY_QC_SIGMA1) ~= height(G)
    meanVar = splitapply(@mean, dataPHY_QC_SIGMA1.(varName), G);
    refTable = table( ...
        LAT, LON, meanVar, ...
        'VariableNames', {'LATITUDE', 'LONGITUDE', varName} ...
        );
else
    refTable = dataPHY_QC_SIGMA1;
end
...
dataStats = summary(dataPHY_QC_SIGMA1);
% and for example see the min, max, and median values of the temperatue
disp("Minimum temperature = " + num2str(dataStats.TEMP.Min) + " degrees");
disp("Maximum temperature = " + num2str(dataStats.TEMP.Max) + " degrees");
disp("Median temperature = " + num2str(dataStats.TEMP.Median) + " degrees");
